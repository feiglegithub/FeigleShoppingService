...
public class BannerDeleteServerlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public BannerDeleteServerlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (null != userName && !("").equals(userName)) {
			if (UserSqlUtils.isBackstageStaff(userName)) {
				...
				boolean flag = BannerSqlUtils.delete(id);
				resp.getWriter().print(flag);
			}else {
				resp.getWriter().print("你不是后台工作人员！");
			}
		}else {
			resp.getWriter().print("用户信息验证不通过，请重新登录！");
		}
	}
